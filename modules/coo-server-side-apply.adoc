...
. A Prometheus resource named `sample-monitoring-stack` is generated in the `coo-demo` namespace. Retrieve the managed fields of the generated Prometheus resource by using command:
+
[source,terminal]
----
...
. Modify a field not controlled by the `MonitoringStack` resource:

.. Change `spec.enforcedSampleLimit`, a field not set by the `MonitoringStack` resource. Create the file `prom-spec-edited.yaml`:
+
...
.. Get the changed Prometheus object and note there is one more section in `managedFields` which has `spec.enforcedSampleLimit`:
+
[source,terminal]
----
...
. Modify a field managed by the `MonitoringStack` resource: 
.. Change `spec.LogLevel`, a field managed by the `MonitoringStack` resource, using the following YAML configuration:
+
[source,yaml]
----
...
.. Confirm that the change has taken place by querying for the log level:
+
[source,terminal]
----
